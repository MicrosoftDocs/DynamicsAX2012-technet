---
title: Technical reference
TOCTitle: Technical reference
ms:assetid: acd93373-ac48-4447-99d4-945c3d5af3fa
ms:mtpsurl: https://technet.microsoft.com/library/Dd362035(v=AX.60)
ms:contentKeyID: 35132721
author: tonyafehr
ms.date: 04/18/2014
mtps_version: v=AX.60
---

# Technical reference 


[!INCLUDE[archive-banner](includes/archive-banner.md)]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

This section contains reference information for system administrators and application users. The following topics are included:

[Setup parameters reference](setup-parameters-reference.md)

[Table and table group reference](table-and-table-group-reference.md)

[Table Permissions Framework reference](table-permissions-framework-reference.md)

[Report catalog for Microsoft Dynamics AX](report-catalog-for-microsoft-dynamics-ax.md)

[Cube and KPI reference for Microsoft Dynamics AX](cube-and-kpi-reference-for-microsoft-dynamics-ax.md)

[License code and configuration key reference](license-code-and-configuration-key-reference.md)

[Security role reference](security-role-reference.md)

[Windows PowerShell for Microsoft Dynamics AX](windows-powershell-for-microsoft-dynamics-ax.md)

  


