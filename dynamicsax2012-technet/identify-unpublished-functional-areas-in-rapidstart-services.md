---
title: Identify unpublished functional areas in RapidStart Services
TOCTitle: Identify unpublished functional areas in RapidStart Services
ms:assetid: b713b495-b5c8-43b0-b020-9cf480742d75
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Dn292560(v=AX.60)
ms:contentKeyID: 55119961
ms.date: 09/18/2015
mtps_version: v=AX.60
---

# Identify unpublished functional areas in RapidStart Services 


_**Applies To:** Microsoft Dynamics AX 2012 R3_

Beginning October 1, 2015, RapidStart services will no longer be supported. For more information, see the announcement on CustomerSourcehttps://mbs.microsoft.com/partnersource/northamerica/news-events/news/RapidStartServicesDiscontinued or PartnerSourcehttps://mbs.microsoft.com/customersource/northamerica/news-events/news-events/news/RapidStartServicesDiscontinued.

In Microsoft Dynamics ERP RapidStart Services, before a functional area can be used in a template as a basis for configuring and implementing an instance of Microsoft Dynamics AX 2012, the functional area must be published. Designers in RapidStart Services can use the **Design** area to identify which functional areas are not currently published.

## Identify unpublished functional areas on the Functional area page

1.  In the RapidStart Services header, click **Design**, and then click **Functional area**.

2.  Above the list of functional areas, click **Unpublished**.

## Identify unpublished functional areas in the Functional areas form

1.  In the RapidStart Services header, click **Design**.

2.  On the **Templates** page, click the **Templates** button, and then select **Add or remove functional area**.
    
    In the **Functional areas** form, unpublished functional areas are indicated by a yellow caution symbol.

## See also

[Learn about published and unpublished templates and functional areas in RapidStart Services](learn-about-published-and-unpublished-templates-and-functional-areas-in-rapidstart-services.md)

[Publish a functional area in RapidStart Services](publish-a-functional-area-in-rapidstart-services.md)

[Unpublish a functional area in RapidStart Services](unpublish-a-functional-area-in-rapidstart-services.md)

[Add a functional area to a template in RapidStart Services](add-a-functional-area-to-a-template-in-rapidstart-services.md)

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

