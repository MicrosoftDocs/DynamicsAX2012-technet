---
title: Delete a batch attribute from an item for a customer
TOCTitle: Delete a batch attribute from an item for a customer
ms:assetid: 90ab7481-9a5f-4510-8292-17e2e733d308
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Hh209372(v=AX.60)
ms:contentKeyID: 36058540
ms.date: 04/18/2014
mtps_version: v=AX.60
audience: Application User
ms.search.region: Global
---

# Delete a batch attribute from an item for a customer 


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

Use this procedure to delete a batch attribute from an item for a customer.

1.  Click **Product information management** \> **Common** \> **Released products**.

2.  Select the item with the batch attribute that you want to delete.

3.  On the **Action Pane**, on the **Manage inventory** tab, in the **Batch attributes** group, click **Customer specific**.

4.  Select the batch attribute line that you want to delete.

5.  Click **Delete**. To delete the attribute, click **Yes** to the **Delete record** prompt.

## See also

[About batch attributes](about-batch-attributes.md)

[Customer specific (form)](https://technet.microsoft.com/en-us/library/hh209393\(v=ax.60\))

  


