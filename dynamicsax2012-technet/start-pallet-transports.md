---
title: Start pallet transports
TOCTitle: Start pallet transports
ms:assetid: 0f18f719-46bb-406f-a993-13351c9de0bf
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Aa496415(v=AX.60)
ms:contentKeyID: 36056016
ms.date: 04/18/2014
mtps_version: v=AX.60
---

# Start pallet transports [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

This topic applies to features in the **Inventory management** module. It does not apply to features in the [Warehouse management](warehouse-management.md) module.

1.  Click **Inventory management** \> **Common** \> **Pallet transports**. Select the pallet transport that you want to start.

2.  Click **Start transport**.

3.  Specify the appropriate options, and then click **OK**.

## See also

[About pallet transports](about-pallet-transports.md)

[Complete pallet transports](complete-pallet-transports.md)

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

