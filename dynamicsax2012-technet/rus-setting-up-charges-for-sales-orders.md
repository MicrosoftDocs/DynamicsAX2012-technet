---
title: (RUS) Setting up charges for sales orders
TOCTitle: (RUS) Setting up charges for sales orders
ms:assetid: 4ca46a8a-e529-4f2a-9199-240f32065e4a
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ733218(v=AX.60)
ms:contentKeyID: 49685186
ms.date: 04/18/2014
mtps_version: v=AX.60
audience: Application User
ms.search.region: Russia
---

# (RUS) Setting up charges for sales orders 


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2_

The following topics provide information about charges for sales orders.

[(RUS) Set up a charges code](rus-set-up-a-charges-code.md)

[(RUS) Set up a transit account to redraw miscellaneous charges to third parties](rus-set-up-a-transit-account-to-redraw-miscellaneous-charges-to-third-parties.md)

[(RUS) Set up a transit tax group to redraw miscellaneous charges to third parties](rus-set-up-a-transit-tax-group-to-redraw-miscellaneous-charges-to-third-parties.md)

[(RUS) Set up a revenue, consumption, and transit account to post sales charges](rus-set-up-a-revenue-consumption-and-transit-account-to-post-sales-charges.md)

  


