---
title: Setting up the requisition process
TOCTitle: Setting up the requisition process
ms:assetid: 7b51b2f5-e443-4299-8275-496d130d91b7
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Hh209264(v=AX.60)
ms:contentKeyID: 36058256
ms.date: 04/18/2014
mtps_version: v=AX.60
f1_keywords:
- purchase requisition
- purchase requisitions
- requisitions
- requisition process
---

# Setting up the requisition process 


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

The topics in this section provide information about how to set up the purchase requisition process.

[About fixed asset determination rules](about-fixed-asset-determination-rules.md)

[About demand consolidation](about-demand-consolidation.md)

[Key tasks: Set up business rules for fixed asset determination](key-tasks-set-up-business-rules-for-fixed-asset-determination.md)

[Configure purchase requisition parameters](configure-purchase-requisition-parameters.md)

[Set up business justification codes](set-up-business-justification-codes.md)

[Set up permissions for ordering products on behalf of someone else](set-up-permissions-for-ordering-products-on-behalf-of-someone-else.md)

[Set up master plans to include requisitions](set-up-master-plans-to-include-requisitions.md)

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

