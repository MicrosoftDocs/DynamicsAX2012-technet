---
title: CommerceActionParameterNames.ReasonCodeLines Field (Microsoft.Dynamics.Retail.RetailServerLibrary)
TOCTitle: ReasonCodeLines Field
ms:assetid: F:Microsoft.Dynamics.Retail.RetailServerLibrary.CommerceActionParameterNames.ReasonCodeLines
ms:mtpsurl: https://technet.microsoft.com/library/microsoft.dynamics.retail.retailserverlibrary.commerceactionparameternames.reasoncodelines(v=AX.60)
ms:contentKeyID: 62203354
author: Khairunj
ms.date: 04/21/2014
mtps_version: v=AX.60
f1_keywords:
- Microsoft.Dynamics.Retail.RetailServerLibrary.CommerceActionParameterNames.ReasonCodeLines
dev_langs:
- CSharp
- C++
- VB
---

# ReasonCodeLines Field


[!INCLUDE[archive-banner](includes/archive-banner.md)]

The reason code lines.

**Namespace:**  [Microsoft.Dynamics.Retail.RetailServerLibrary](microsoft-dynamics-retail-retailserverlibrary-namespace.md)  
**Assembly:**  Microsoft.Dynamics.Retail.RetailServerLibrary (in Microsoft.Dynamics.Retail.RetailServerLibrary.dll)

## Syntax

``` vb
'Declaration
Public Const ReasonCodeLines As String
'Usage
Dim value As String

value = CommerceActionParameterNames.ReasonCodeLines
```

``` csharp
public const string ReasonCodeLines
```

``` c++
public:
literal String^ ReasonCodeLines
```

## See Also

#### Reference

[CommerceActionParameterNames Class](commerceactionparameternames-class-microsoft-dynamics-retail-retailserverlibrary.md)

[Microsoft.Dynamics.Retail.RetailServerLibrary Namespace](microsoft-dynamics-retail-retailserverlibrary-namespace.md)

