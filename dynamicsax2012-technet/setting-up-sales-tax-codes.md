---
title: Setting up sales tax codes
TOCTitle: Setting up sales tax codes
ms:assetid: 05802d5c-cb62-4b96-80ad-2b80b2fbe1dc
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg230668(v=AX.60)
ms:contentKeyID: 36055954
ms.date: 04/18/2014
mtps_version: v=AX.60
f1_keywords:
- setting up sales tax codes
- sales tax code
- tax
- tax code
- tax setup
---

# Setting up sales tax codes [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

Every company collects and pays taxes to various tax authorities. The rules and rates vary by country/region, state, county, and city. In addition, the rules must be updated periodically when tax authorities change their requirements.

Sales tax codes contain the basic information about how much you collect and pay to the authorities. When you set up sales tax codes, you define the amounts or percentages that must be collected. You also define the various methods by which those amounts or percentages are applied to transaction amounts.

The topics in this section provide information about how to set up sales tax codes for the methods and rates that your tax authorities require.

[About the Marginal base field](about-the-marginal-base-field.md)

[About the sales tax calculation methods in the Origin field](about-the-sales-tax-calculation-methods-in-the-origin-field.md)

[About the Whole amount and Interval options for sales tax codes](about-the-whole-amount-and-interval-options-for-sales-tax-codes.md)

[Set up and use sales tax codes](set-up-and-use-sales-tax-codes.md)

[Create various kinds of sales tax codes](create-various-kinds-of-sales-tax-codes.md)

[Set up a sales tax on a sales tax](set-up-a-sales-tax-on-a-sales-tax.md)

[Set up packing duty taxes for packing materials](set-up-packing-duty-taxes-for-packing-materials.md)

[(MEX) Set up tax report category](mex-set-up-tax-report-category.md)

[(NOR) Attach reporting codes to sales tax codes](nor-attach-reporting-codes-to-sales-tax-codes.md)

[(SGP) Set up tax codes for GST reporting](sgp-set-up-tax-codes-for-gst-reporting.md)

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

