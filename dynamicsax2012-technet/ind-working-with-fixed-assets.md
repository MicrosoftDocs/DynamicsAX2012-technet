---
title: (IND) Working with Fixed assets
TOCTitle: (IND) Working with Fixed assets
ms:assetid: d2200f97-5db5-4001-8881-f982e4b2cb69
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ664904(v=AX.60)
ms:contentKeyID: 49386232
ms.date: 04/18/2014
mtps_version: v=AX.60
---

# (IND) Working with Fixed assets [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2_

This section provides information about working with the Fixed assets business process components.

[(IND) Revalue assets overview](ind-revalue-assets-overview.md)

[(IND) Depreciate/amortize assets overview](ind-depreciate-amortize-assets-overview.md)

[(IND) Capitalize assets overview](ind-capitalize-assets-overview.md)

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

