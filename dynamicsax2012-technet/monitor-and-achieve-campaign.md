---
title: Monitor and achieve campaign
TOCTitle: Monitor and achieve campaign
ms:assetid: e38c2001-d736-4dbf-917a-ac8ec3970999
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg243218(v=AX.60)
ms:contentKeyID: 36059723
ms.date: 04/18/2014
mtps_version: v=AX.60
f1_keywords:
- campaigns
- campaign
audience: Application User
ms.search.region: Global
---

# Monitor and achieve campaign 


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

The topics in this section provide information about how to monitor and achieve the goals of your campaign.

[Changing campaign activity data](changing-campaign-activity-data.md)

[Changing campaign target activity data](changing-campaign-target-activity-data.md)

[Changing telemarketing activity data](changing-telemarketing-activity-data.md)

[Add campaign targets](add-campaign-targets.md)

[Change status on the call list](change-status-on-the-call-list.md)

  


