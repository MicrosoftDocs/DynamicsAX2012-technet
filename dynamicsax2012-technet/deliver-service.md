---
title: Deliver service
TOCTitle: Deliver service
ms:assetid: dcdbb60c-aeac-4054-926e-75c3943b31a5
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Gg243195(v=AX.60)
ms:contentKeyID: 36059676
ms.date: 04/18/2014
mtps_version: v=AX.60
f1_keywords:
- service
- customer
- agreement
audience: Application User
ms.search.region: Global
---

# Deliver service 


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

The topics in this section provide information about how to deliver a service.

[Credit subscription transactions](credit-subscription-transactions.md)

[Create subscription fee transactions](create-subscription-fee-transactions.md)

[About accruing subscriptions](about-accruing-subscriptions.md)

[Move service order lines](move-service-order-lines.md)

[Cancel service orders](cancel-service-orders.md)

[Create service orders manually](create-service-orders-manually.md)

[Accrue subscription revenue](accrue-subscription-revenue.md)

  


