---
title: (CHN) Inventory aging report
TOCTitle: (CHN) Inventory aging report
ms:assetid: 78156e1e-083a-49b2-bf32-afa9fc42314b
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Dn169462(v=AX.60)
ms:contentKeyID: 53865952
ms.date: 04/18/2014
mtps_version: v=AX.60
---

# (CHN) Inventory aging report [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

Generate and print a report that displays the on-hand inventory amount for a selected item and the related aging periods for the item.

## How to filter the data on this report

When you generate this report, the following default parameters are displayed. You can use these parameters to filter the data that will be displayed on the report. For more information, see [Filter the data on a report](filter-the-data-on-a-report.md).

<table>
<colgroup>
<col style="width: 50%" />
<col style="width: 50%" />
</colgroup>
<thead>
<tr class="header">
<th><p>Field</p></th>
<th><p>Description</p></th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td><p><strong>As on date:</strong></p></td>
<td><p>Select the date for which the information on this report should be generated.</p></td>
</tr>
<tr class="even">
<td><p><strong>Warehouse:</strong></p></td>
<td><p>Select the warehouse to include on the report.</p></td>
</tr>
<tr class="odd">
<td><p><strong>Item number:</strong></p></td>
<td><p>Select the item to include on the report.</p></td>
</tr>
<tr class="even">
<td><p><strong>Item group number:</strong></p></td>
<td><p>For the selected item, select the assigned group number to include on the report.</p></td>
</tr>
<tr class="odd">
<td><p><strong>Aging period 1 (Day)</strong></p></td>
<td><p>Enter the number that represents the interval for the size or length of aging period 1.</p></td>
</tr>
<tr class="even">
<td><p><strong>Aging period 2 (Day)</strong></p></td>
<td><p>Enter the number that represents the interval for the size or length of aging period 2.</p></td>
</tr>
<tr class="odd">
<td><p><strong>Aging period 3 (Day)</strong></p></td>
<td><p>Enter the number that represents the interval for the size or length of aging period 3.</p></td>
</tr>
<tr class="even">
<td><p><strong>Aging period 4 (Day)</strong></p></td>
<td><p>Enter the number that represents the interval for the size or length of aging period 4.</p></td>
</tr>
</tbody>
</table>


## How to work with reports

The following topics explain how to print a report and how to filter and sort the data on a report.

  - [Print or email a report](print-or-email-a-report.md)

  - [Filter the data on a report](filter-the-data-on-a-report.md)

  - [Sort the data on a report](sort-the-data-on-a-report.md)

## Details of this report

The following table explains where to find the report in the Application Object Tree (AOT) and how to navigate to the report in the Microsoft Dynamics AX client.

<table>
<colgroup>
<col style="width: 50%" />
<col style="width: 50%" />
</colgroup>
<thead>
<tr class="header">
<th><p>Detail</p></th>
<th><p>Description</p></th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td><p>Name of report in the AOT</p></td>
<td><p>InventAging_CN</p></td>
</tr>
<tr class="even">
<td><p>Location of report in the AOT</p></td>
<td><p>SSRS Reports\Reports\InventAging_CN</p></td>
</tr>
<tr class="odd">
<td><p>Menu item of the report</p></td>
<td><p>InventAging_CN</p></td>
</tr>
<tr class="even">
<td><p>Navigation to the report</p></td>
<td><p><strong>Navigation Path Not Found</strong></p></td>
</tr>
</tbody>
</table>


## Where the data in this report comes from

The data on this report comes from the following sources:

  - InventAgingTmp\_CN

If you are a developer, you can learn more about where the data on a report comes from by using the following procedure.

1.  Open the AOT.

2.  Locate the report in the **SSRS Reports**\\**Reports** node.

3.  Right-click the report and click **Add-Ins** \> **Cross-reference** \> **Using (instant view)**.

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

