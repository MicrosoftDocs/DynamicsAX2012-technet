...
f1_keywords:
- Microsoft.Dynamics.Commerce.Runtime.TransactionService.TransactionServiceClient.GetManagerActivityHistory
dev_langs:
- CSharp
- C++
- VB
...
# GetManagerActivityHistory Method
...
``` vb
'Declaration
Public Function GetManagerActivityHistory ( _
    storeIds As String(), _
    activityTypes As EmployeeActivityType(), _
...
Dim activityTypes As EmployeeActivityType()
...
returnValue = instance.GetManagerActivityHistory(storeIds, _
    activityTypes, breakActivities, _
    fromUtcDateTime, toUtcDateTime, _
    pagingInfo, sortingInfo)
```

``` csharp
public PagedResult<EmployeeActivity> GetManagerActivityHistory(
    string[] storeIds,
    EmployeeActivityType[] activityTypes,
...
``` c++
public:
PagedResult<EmployeeActivity^>^ GetManagerActivityHistory(
    array<String^>^ storeIds, 
    array<EmployeeActivityType>^ activityTypes, 
