---
title: DataProtectionRequestHandler.Execute Method  (Microsoft.Dynamics.Commerce.Runtime.Services)
TOCTitle: Execute Method
ms:assetid: Overload:Microsoft.Dynamics.Commerce.Runtime.Services.DataProtectionRequestHandler.Execute
ms:mtpsurl: https://technet.microsoft.com/library/microsoft.dynamics.commerce.runtime.services.dataprotectionrequesthandler.execute(v=AX.60)
ms:contentKeyID: 65316876
author: Khairunj
ms.date: 05/18/2015
mtps_version: v=AX.60
f1_keywords:
- Microsoft.Dynamics.Commerce.Runtime.Services.DataProtectionRequestHandler.Execute
dev_langs:
- CSharp
- C++
- VB
---

# Execute Method

## Overload List

<table>
<thead>
<tr class="header">
<th> </th>
<th>Name</th>
<th>Description</th>
</tr>
</thead>
<tbody>
<tr class="odd">
<td><img src="images/Dn990315.pub(en-us,AX.60).gif" title="Dn998443.pub(en-us,AX.60).gif" alt="Dn998443.pub(en-us,AX.60).gif" /></td>
<td><a href="dataprotectionrequesthandler-execute-method-request-microsoft-dynamics-commerce-runtime-services.md">Execute(Request)</a></td>
<td></td>
</tr>
<tr class="even">
<td><img src="images/Dn990315.pub(en-us,AX.60).gif" title="Dn998443.pub(en-us,AX.60).gif" alt="Dn998443.pub(en-us,AX.60).gif" /></td>
<td><a href="dataprotectionrequesthandler-execute-method-request-microsoft-dynamics-commerce-runtime-services.md">Execute(Request)</a></td>
<td></td>
</tr>
<tr class="odd">
<td><img src="images/Dn990315.pub(en-us,AX.60).gif" title="Dn998443.pub(en-us,AX.60).gif" alt="Dn998443.pub(en-us,AX.60).gif" /></td>
<td><a href="dataprotectionrequesthandler-execute-method-request-microsoft-dynamics-commerce-runtime-services.md">Execute(Request)</a></td>
<td></td>
</tr>
<tr class="even">
<td><img src="images/Dn987397.pubmethod(en-us,AX.60).gif" title="Public method" alt="Public method" /></td>
<td><a href="dataprotectionrequesthandler-execute-method-request-microsoft-dynamics-commerce-runtime-services.md">Execute(Request)</a></td>
<td>Represents the entry point of the request handler.</td>
</tr>
</tbody>
</table>


Top

## See Also

#### Reference

[DataProtectionRequestHandler Class](dataprotectionrequesthandler-class-microsoft-dynamics-commerce-runtime-services.md)

[Microsoft.Dynamics.Commerce.Runtime.Services Namespace](microsoft-dynamics-commerce-runtime-services-namespace.md)

