---
title: Set up an asset group depreciation book
TOCTitle: Set up an asset group depreciation book
ms:assetid: 88c5ec86-8097-469b-9d5b-4b43e5b6e3ab
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Aa498229(v=AX.60)
ms:contentKeyID: 36058457
ms.date: 04/18/2014
mtps_version: v=AX.60
---

# Set up an asset group depreciation book [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

You can use the **Fixed asset group/depreciation book** form to associate depreciation books to fixed asset groups.

1.  Click **Fixed assets** \> **Setup** \> **Fixed asset groups**.

2.  Click **Depreciation books**.

3.  In the **Depreciation book** field, select a depreciation book that you want to associate with the fixed asset group.

## See also

[Fixed asset groups (form)](https://technet.microsoft.com/en-us/library/aa573347\(v=ax.60\))

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

