---
title: (POL) Setting up and maintaining Accounts receivable
TOCTitle: (POL) Setting up and maintaining Accounts receivable
ms:assetid: 84ee9611-1cff-4177-b03f-8f95cc03bd71
ms:mtpsurl: https://technet.microsoft.com/en-us/library/JJ678268(v=AX.60)
ms:contentKeyID: 49386990
ms.date: 04/18/2014
mtps_version: v=AX.60
---

# (POL) Setting up and maintaining Accounts receivable [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2_

The topics in this section provide information about setting up and maintaining Accounts receivable.

[(POL) Configuring interest and finance charges](pol-configuring-interest-and-finance-charges.md)

[(POL) Set up credit note parameters](pol-set-up-credit-note-parameters.md)

[(POL) Configuring required Accounts receivable information](pol-configuring-required-accounts-receivable-information.md)

[(POL) Managing customer information](pol-managing-customer-information.md)

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

