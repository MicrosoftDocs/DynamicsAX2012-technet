---
title: Setting up transportation
TOCTitle: Setting up transportation
ms:assetid: 70ce7a29-3398-43a2-aea1-ea313c6228f6
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Dn553169(v=AX.60)
ms:contentKeyID: 62200092
ms.date: 04/23/2014
mtps_version: v=AX.60
---

# Setting up transportation [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2_


> [!NOTE]
> <P>This topic applies to features in the <STRONG>Transportation management</STRONG> module. It does not apply to features in the <A href="inventory-management.md">Inventory management</A> module.</P>



The topics in this section provide information about how to set up parameters for transportation management.

[Set up accessorial assignments](set-up-accessorial-assignments.md)

[Set up a transportation status](set-up-a-transportation-status.md)

[Set up carrier fuel indexes](set-up-carrier-fuel-indexes.md)

[Set up a load template](set-up-a-load-template.md)

[Set up a transportation template](set-up-a-transportation-template.md)

[Set up a transportation tender](set-up-a-transportation-tender.md)

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

