---
title: (NOR) Configuring customer payment formats
TOCTitle: (NOR) Configuring customer payment formats
ms:assetid: 61d094f1-75b1-4210-9e37-8c98558b0a9f
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Hh209184(v=AX.60)
ms:contentKeyID: 36057659
ms.date: 04/18/2014
mtps_version: v=AX.60
---

# (NOR) Configuring customer payment formats [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

The following topics provide information about configuring customer payment formats.

[(NOR) Set up a method of payment for eInvoice](nor-set-up-a-method-of-payment-for-einvoice.md)

[(NOR) Set up the OCR method of payment](nor-set-up-the-ocr-method-of-payment.md)

[(NOR) Set up the Avtalegiro method of payment](nor-set-up-the-avtalegiro-method-of-payment.md)

[(NOR) Set up the eGiro method of payment](nor-set-up-the-egiro-method-of-payment.md)

[(NOR) Set up the Telepay 2.01 method of payment](nor-set-up-the-telepay-2-01-method-of-payment.md)

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

