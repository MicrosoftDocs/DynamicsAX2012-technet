---
title: Delete posted inventory journals
TOCTitle: Delete posted inventory journals
ms:assetid: 27975d05-9bd6-4d59-8a66-8e9d9287f532
ms:mtpsurl: https://technet.microsoft.com/en-us/library/Aa496832(v=AX.60)
ms:contentKeyID: 36056212
ms.date: 04/18/2014
mtps_version: v=AX.60
---

# Delete posted inventory journals [AX 2012]


_**Applies To:** Microsoft Dynamics AX 2012 R3, Microsoft Dynamics AX 2012 R2, Microsoft Dynamics AX 2012 Feature Pack, Microsoft Dynamics AX 2012_

1.  Click **Inventory management** \> **Periodic** \> **Clean up** \> **Inventory journals cleanup**.

2.  Select the types of inventory journals to delete.

3.  Click the **Batch** tab if you want to define parameters to delete old inventory journals regularly.

4.  Click **OK** to run the deletion process.

## See also

[Inventory journal (form)](https://technet.microsoft.com/en-us/library/aa558607\(v=ax.60\))

  
**Announcements:** To see known issues and recent fixes, use [Issue search](http://go.microsoft.com/fwlink/?linkid=389258) in [Microsoft Dynamics Lifecycle Services](http://go.microsoft.com/fwlink/?linkid=306505) (LCS).

